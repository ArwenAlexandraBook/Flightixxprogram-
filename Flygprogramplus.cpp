ubuntu@evalassignment:/mnt/eval/guardianbardess@gmail.com/Flightixxprogram-$ cat Flygprogramplus.cpp
...
public: /*Public acces specifier*/
	...
	int getfseat(int p) { return( this->fcflags[p] ); }
...
public:
	bool startPrintJob(list<Flight *> flights, list<Booking *> bookings)
	{
		...
		for(bit = bookings.begin(); bit != bookings.end(); bit++) {
			// search for flight for booking
			for(fit = flights.begin(); fit != flights.end(); fit++) {
				// could it be this flight?
				if( 
		...
		return true;
       	...
       	// function that allocates seats to the different bookings
	bool allocate_seat(Flight *f,string inclass,int *row, int *seat)  
	{
		// if a seat in the "inclass" is free, then
		// allocate it, set row,seat  and return true
		if( inclass == "first" ) {
			for(int p=0; p < f->getnrfcseats(); p++) {
				if( f->getfseat(p) == 0 ) { // its free
					f->bookfseat(p); // mark it taken
					*row = p/7+1;
					*seat = p%7 + 1;
					return( true ); // we found a seat
				}
			}
		}
		...
		return( false ); // we  did not find a seat
	}
	...
	void print_ticket(Flight *f, Booking *b,int row, int seat)
	{
		...
		ofstream ticket_file(filename); /*Opens the file*/
		if (ticket_file.is_open()) 
		{
			ticket_file << "BOOKING:\n" << b->getbookingnr() <<endl;
			ticket_file << " FLIGHT:" << f->getfligthnr(); 
			...
			ticket_file << " ROW:" << row << "" << " SEAT:" << seat <<endl;  /*prints information to the ticket file*/
